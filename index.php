...
$pricingCatalog = [
    'landing-page' => [
        'label' => 'Landing Page',
        'description' => 'Satu halaman fokus untuk memperkenalkan layanan, produk, atau campaign bisnis Anda.',
        'plans' => [
    ...
    'company-profile' => [
        ...
        'description' => 'Bangun kredibilitas perusahaan melalui profil, layanan, portofolio, tim, dan kontak yang profesional.',
        'plans' => [
    ...
    'toko-online' => [
        'label' => 'Toko Online',
        'description' => 'Mulai menjual produk secara online dengan katalog, keranjang belanja, checkout, dan fitur bisnis dasar.',
        'plans' => [
    ...
    'e-commerce' => [
        'label' => 'E-Commerce',
        'description' => 'Solusi toko digital lebih lengkap untuk bisnis yang membutuhkan katalog, transaksi, dan operasional online yang berkembang.',
        'plans' => [
    ...
    'link-bio' => [
        'label' => 'Link Bio',
        'description' => 'Kumpulkan semua link penting bisnis, sosial media, katalog, dan kontak dalam satu halaman yang rapi.',
        'plans' => [
...
<body>
    ...
    <main id="main-content">
        ...
        <section class="section portfolio-section" id="portfolio"><div class="container"><div class="section-heading reveal"><p class="eyebrow">Portfolio jasa website</p><h2>Website yang <span>berbicara</span></h2></div><div class="portfolio-grid"><?php foreach ($projects as $index => $project): ?><article class="project-card reveal <?= $index % 2 ? 'project-offset' : '' ?>"><?php if (!empty($project['preview_url'])): ?><div class="project-image project-preview"><iframe src="<?= e($project['preview_url']) ?>" title="Preview hero website <?= e($project['name']) ?>" loading="lazy" scrolling="no" referrerpolicy="strict-origin-when-cross-origin"></iframe><a class="project-preview-link" href="<?= e($project['url']) ?>" target="_blank" rel="noopener">Buka Website <span aria-hidden="true">↗</span></a></div><?php else: ?><a class="project-image" href="<?= e($project['url']) ?>" target="_blank" rel="noopener"><img src="<?= e($project['image']) ?>" alt="Preview website <?= e($project['name']) ?>" loading="lazy"><span class="project-arrow" aria-hidden="true">↗</span></a><?php endif; ?><div class="project-meta"><h3><?= e($project['name']) ?></h3><a href="<?= e($project['url']) ?>" target="_blank" rel="noopener">Visit Website <span aria-hidden="true">↗</span></a></div></article><?php endforeach; ?></div></div></section>
        <section class="section pricing-section" id="pricing"><span id="pricelist" class="anchor-target"></span><div class="container"><div class="section-heading reveal"><p class="eyebrow">Harga jasa website</p><h2>Pilih paket jasa pembuatan <span>website</span></h2></div><div class="pricing-selector reveal"><div class="pricing-selector-field"><label for="pricing-type">Jenis website</label><select id="pricing-type" data-pricing-select aria-controls="pricing-grid"><?php foreach ($pricingCatalog as $key => $category): ?><option value="<?= e($key) ?>" <?= $key === $defaultPricingKey ? 'selected' : '' ?>><?= e($category['label']) ?></option><?php endforeach; ?></select></div><p class="pricing-selector-description" data-pricing-description aria-live="polite"><?= e($defaultPricing['description']) ?></p></div><div class="pricing-grid" id="pricing-grid" data-pricing-grid><?php foreach ($defaultPricing['plans'] as $index => $plan): ?><article class="price-card <?= $index === 1 ? 'price-card-featured' : '' ?> reveal reveal-delay-<?= min($index, 2) ?>"><div class="price-header"><h3><?= e($plan['name']) ?></h3></div><strong class="price">Rp<?= e($plan['price']) ?></strong><p class="renewal"><?= e($plan['renewal']) ?></p><ul><?php foreach ($plan['features'] as $feature): ?><li><span aria-hidden="true">✓</span><?= e($feature) ?></li><?php endforeach; ?></ul><a class="button <?= $index === 1 ? 'button-primary' : 'button-outline' ?>" href="<?= e(wa('Halo Webkubator, saya mau order ' . $defaultPricing['label'] . '. Nama: Bidang Bisnis: Paket: ' . $plan['name'] . ' Jenis Website: Terimakasih')) ?>" target="_blank" rel="noopener">Pesan Sekarang <span aria-hidden="true">↗</span></a></article><?php endforeach; ?></div><script id="pricing-catalog" type="application/json"><?= json_encode($pricingCatalog, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?></script></div></section>

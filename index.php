...
<body>
    ...
    <main id="main-content">
        ...
        <section class="section pricing-section" id="pricing"><span id="pricelist" class="anchor-target"></span><div class="container"><div class="section-heading reveal"><p class="eyebrow">Harga jasa website</p><h2>Paket jasa pembuatan <span>website</span></h2></div><div class="pricing-grid"><?php foreach ($plans as $index => $plan): ?><article class="price-card <?= $index === 1 ? 'price-card-featured' : '' ?> reveal reveal-delay-<?= min($index, 2) ?>"><div class="price-header"><h3><?= e($plan['name']) ?></h3></div><strong class="price">Rp<?= e($plan['price']) ?></strong><p class="renewal"><?= e($plan['renewal']) ?></p><ul><?php foreach ($plan['features'] as $feature): ?><li><span aria-hidden="true">✓</span><?= e($feature) ?></li><?php endforeach; ?></ul><a class="button <?= $index === 1 ? 'button-primary' : 'button-outline' ?>" href="<?= e(wa('Halo Webkubator, saya mau order paket website. Nama: Bidang Bisnis: Paket: ' . $plan['name'] . ' Jenis Website: Terimakasih')) ?>" target="_blank" rel="noopener">Pesan Sekarang <span aria-hidden="true">↗</span></a></article><?php endforeach; ?></div></div></section>

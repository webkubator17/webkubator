(() => {
  ...
  function render() {
    ...
    const empty = items.length === 0; $('cart-empty').hidden = !empty; $('cart-clear').hidden = empty; $('cart-list-title').textContent = empty ? 'Produk pilihan' : `Produk pilihan (${items.length})`; if ($('cart-item-count')) $('cart-item-count').textContent = `${totalQuantity} item${totalQuantity === 1 ? '' : ''}`;
